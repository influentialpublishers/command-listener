...
function RabbitMq({ channel, name, durable=true, prefetch=1, deadLetterExchange=null }) {

  const options = R.reject(
    R.isNil
  , { name, durable, prefetch, deadLetterExchange }
  )

  return {
    listen : listen.bind(null, channel, options)
